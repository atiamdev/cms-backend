 ...
 */
async function generateMonthlyInvoices({ periodYear, periodMonth, branchId, studentId, initiatedBy, consolidate = true } = {}) {
  ...
  if (consolidate) {
    ...
    const studentsWithInvoices = new Set(existingInvoices.map((e) => String(e.studentId)));
    ...
    for (const course of courses) {
      ...
      for (const stu of students) {
        ...
        if (studentsWithInvoices.has(studentKey)) {
          skipped.push({ courseId: course._id, studentId: stu._id, reason: 'has_invoice' });
          continue;
        }

        const amount = fs.perPeriodAmount !== undefined && fs.perPeriodAmount !== null 
          ? fs.perPeriodAmount 
          : (fs.components?.reduce((s,c)=>s+(c.amount||0),0) || fs.totalAmount || 0);
        ...
        invoiceData.feeComponents.push(...(fs.components || fs.feeComponents || []));
    ...
    for (const [studentId, data] of studentInvoiceMap) {
      ...
      const scholarshipAmount = data.scholarshipPercentage > 0
    ...
    console.log(`Preparing to create ${invoicesToCreate.length} consolidated invoices`);

    if (invoicesToCreate.length > 0) {
      try {
        const result = await Fee.insertMany(invoicesToCreate, { ordered: false });
        ...
        for (const fee of result) {
          try {
            ...
            console.error(`Error applying credit to invoice ${fee._id}:`, creditErr.message);
            // Continue processing other invoices even if credit application fails
          }
        }

        // Prepare notifications
        const monthName = new Date(periodYear, periodMonth - 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
        for (const fee of result) {
          notifications.push({
        ...
        if (err && err.insertedDocs) {
          console.log(`Partial success: ${err.insertedDocs.length} invoices created despite error`);
    ...
    for (const course of courses) {
      ...
      const students = await Student.find(studentQuery).select("_id branchId scholarshipPercentage");
      ...
      for (const stu of students) {
        ...
        // Calculate scholarship amount if student has active scholarship
        const scholarshipAmount = stu.scholarshipPercentage > 0
      ...
      if (invoicesToCreate.length > 0) {
        try {
          ...
          console.error("Error inserting invoices for course", course._id, err.message);
        }
      }
    }
  }
  ...
  if (notifications.length > 0) {
    try {
      ...
      console.log(`Notifications sent: ${result.successful}/${result.total} successful`);
    } catch (err) {
 ...
 */
async function generateInvoicesForFrequency({ frequency, date = new Date(), branchId, initiatedBy } = {}) {
  const createdFees = [];
  const skipped = [];
  const notifications = [];
  ...
  for (const course of courses) {
    ...
    const existingInvoices = await Fee.find({ courseId: course._id, periodStart }).select("studentId");
    const existingSet = new Set(existingInvoices.map((e) => String(e.studentId)));
    ...
    if (invoicesToCreate.length > 0) {
      try {
        ...
        // Prepare notifications
        const periodName = periodStart.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
        for (const fee of result) {
          notifications.push({
            studentId: fee.studentId,
            feeId: fee._id,
  ...
  if (notifications.length > 0) {
    try {
      const result = await notifyStudentsOfInvoices(notifications);
      notificationsSent = result.successful;
      console.log(`${frequency} invoice notifications: ${result.successful}/${result.total} successful`);
    } catch (err) {
      console.error("Error sending invoice notifications:", err);
    }
  }
 ...
 */
async function createInvoiceForEnrollment({ studentId, course, date = new Date(), initiatedBy = null } = {}) {
  if (!course || !course.feeStructure) return { created: 0, reason: 'no_fee_structure' };

  const fs = course.feeStructure;
  if (!fs.billingFrequency || fs.billingFrequency === 'term') return { created: 0, reason: 'not_periodic' };
  if (!fs.createInvoiceOnEnrollment) return { created: 0, reason: 'not_configured' };

  // Get student scholarship info
  const student = await Student.findById(studentId).select('scholarshipPercentage');
  ...
  // Determine amount
  const amount = fs.perPeriodAmount !== undefined && fs.perPeriodAmount !== null ? fs.perPeriodAmount : fs.components?.reduce((s,c)=>s+(c.amount||0),0) || fs.totalAmount || 0;
  ...
  const existing = await Fee.findOne({ studentId, courseId: course._id, periodStart });
  if (existing) return { created: 0, reason: 'exists' };

  // Calculate scholarship amount if student has active scholarship
  const scholarshipAmount = scholarshipPercentage > 0
  ...
  notifyStudentsOfInvoices([{
    ...
    period: periodName,
    branchId: course.branchId,
  }]).catch(err => console.error("Error sending enrollment invoice notification:", err));

  return { created: 1, feeId: feeDoc._id };
}

module.exports = { generateMonthlyInvoices, generateInvoicesForFrequency, createInvoiceForEnrollment };
