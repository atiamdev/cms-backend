...
class BioTimeService {
   ...
   */
  async updateEmployeeAccess(employeeId, feeStatus) {
    const data = {
  ...
  async syncStudent(studentData) {
    try {
      ...
      if (existingStudents.data && existingStudents.data.length > 0) {
        ...
        console.log(`Updated student ${studentData.studentId} in BioTime`);
        return bioTimeEmployee.id;
      } else {
        // Create new student
        const result = await this.createEmployee(studentData);
        console.log(`Created student ${studentData.studentId} in BioTime`);
        return result.id;
      }
    } catch (error) {
      console.error(
   ...
   */
  async updateStudentAccess(studentId, feeStatus) {
    try {
      const existingStudents = await this.getEmployees({ emp_code: studentId });

      if (existingStudents.data && existingStudents.data.length > 0) {
        const bioTimeEmployee = existingStudents.data[0];
        await this.updateEmployeeAccess(bioTimeEmployee.id, feeStatus);

        // Resync to device to apply changes immediately
        await this.resyncEmployeeToDevice([bioTimeEmployee.id]);

        console.log(`Updated access for student ${studentId} to ${feeStatus}`);
        return true;
      } else {
